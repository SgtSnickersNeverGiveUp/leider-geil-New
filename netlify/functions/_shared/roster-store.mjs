...
export const ROSTER_STORE_NAME = "roster";
...
export function getRosterStore() {
  return getStore(ROSTER_STORE_NAME);
}

export async function seedRosterIfEmpty(store = getRosterStore()) {
...
export async function listRoster(store = getRosterStore()) {
  await seedRosterIfEmpty(store);
  const { blobs } = await store.list();
  const members = [];
